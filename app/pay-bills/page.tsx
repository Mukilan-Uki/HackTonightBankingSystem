...
type Account = { account_number: string; account_name: string; balance: string }
...
export default function PayBillsPage() {
  ...
  const selectedAccountObj = myAccounts.find((a) => a.account_number === fromAccount)
  const currentBalance = selectedAccountObj ? Number(selectedAccountObj.balance) : 0
  ...
  async function handlePayNow() {
    ...
    if (!fromAccount) {
      setFailReason('Please select an account to pay from')
      setScreen('failed')
      return
    }
    ...
    setPaying(true)
    try {
      // Use admin vault (9999999999) as the biller destination account
      const res = await fetch('/api/transfer', {
        ...
        body: JSON.stringify({
          ...
          amount,
          description: `Bill payment: ${selectedBiller?.name} | Ref: ${billId}`,
        }),
      })
      ...
      // Fallback: if DB not available, do client-side simulation
      const confNum = Math.floor(10000000 + Math.random() * 90000000).toString()
      setConfirmationNumber(confNum)
      setScreen('success')
    } finally {
              ...
              <div className="card">
                ...
                <div className="field">
                  ...
                  <select
                    ...
                    style={{ background: '#f3f4f6', border: '1.5px solid transparent', borderRadius: 12, padding: '0.85rem 1.1rem', fontSize: '0.95rem', color: '#333', outline: 'none', width: '100%' }}
                  >
                    {myAccounts.length === 0 && <option value="">Loading accounts…</option>}
                    {myAccounts.map((a) => (
                      <option key={a.account_number} value={a.account_number}>
                        {a.account_name} — Rs.{Number(a.balance).toLocaleString()}
                      </option>
                    ))}
                  </select>
                </div>
