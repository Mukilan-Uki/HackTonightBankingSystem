...
type Account = { account_number: string; account_name: string; balance: string; full_name: string }
type Transaction = { id: number; from_account: string; to_account: string; amount: string; description: string; created_at: string }
...
export default function Dashboard() {
    ...
    async function load() {
      try {
        ...
        if (accData.accounts[0]) {
          const txRes = await fetch(`/api/transactions?account=${accData.accounts[0].account_number}`, { credentials: 'include' })
          const txData = await txRes.json()
          if (txData.ok) setTransactions(txData.transactions.slice(0, 5))
        }
      } catch { /* DB offline, show empty state */ } finally {
        setLoading(false)
      }
    }
    load()
  ...
  const userName = primaryAccount?.full_name?.split(' ')[0] || 'there'
  const myAccountNums = new Set(accounts.map((a) => a.account_number))
        ...
        <div className="top-section">
          <div className="welcome-card">
            ...
            <div className="balance-card">
              ...
              <p className="balance-amount">Rs. {totalBalance.toLocaleString()}</p>
        ...
        <div className="transactions-section">
          ...
          <div className="transactions-card">
            {transactions.length === 0 ? (
              <p className="text-gray-600">No recent transactions yet.</p>
            ) : (
              transactions.map((t) => (
                <div key={t.id} className="transaction-item">
                  <img src="/person-logo.png" alt="user" className="avatar" />
                  <span className="transaction-date">{new Date(t.created_at).toLocaleDateString()}</span>
                  <span className="transaction-account">{t.from_account} → {t.to_account}</span>
                  <span className="transaction-amount">Rs. {Number(t.amount).toLocaleString()}</span>
